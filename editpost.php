...
if(empty($Title)){
    $_SESSION["ErrorMessage"]="The Title Must Be Filled Out";
    Redirect_to("addnewpost.php");
    
}elseif(strlen($Title)<4){
    $_SESSION["ErrorMessage"]="Short name input atlist more than 4 charatres";
    Redirect_to("addnewpost.php");
    
    
}else{
    global $con;
    $EdithFromURL=$_GET[Edit];
    $Query="UPDATE admin_panel SET datetime='$datetime', title='$Title', category='$Category', author='$Admin', image='$Image', post='$Post' WHERE Id='$EdithFromURL'";

    $Execute=mysql_query($Query);
    move_uploaded_file($_FILES["image"] ["tmp_name"],$Target);
    if($Execute){
    $_SESSION["SuccessMessage"]="Post is Updated Successfully";
...
<div class="col-sm-10">
    ...
    <div>
        <?php
        $searchQuerypareameter=$_GET['Edit'];
        $con;
        ...
        ?>
        <form action="editpost.php?Edit=<?php echo  $searchQuerypareameter; ?>" method="post" enctype="multipart/form-data">
         <fieldset>
                <div class="form-group">
                 <label for="title"><span class="fieldinfo">Title:</span></label>
                 <input  value="<?php echo $TitleToBeUpdated; ?>" class="form-control" type="text"  name="Title" id="title"  placeholder="Make Your Title Change">
                    </div>
              ...
              <div class="form-group">
                        ...
                        <textarea rows="10"  class="form-control" name="POST" id="postarea">
                    ...
                    <br>
                    <input class="btn btn-success btn-block" type="submit" name="submit" value="Update New Post">
                </fieldset>
         </form>
     </div>
